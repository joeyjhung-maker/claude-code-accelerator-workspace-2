...
### Phases 3 + 4: Deep-Dives and Landing Pages (RUN IN PARALLEL)

Phases 3 and 4 are independent of each other, and each deep-dive is independent of the others. Do NOT run them sequentially. After Phase 2 completes, launch ALL of the following subagents in a SINGLE message using the Agent tool (subagent_type: general-purpose), so they run concurrently:

**Deep-dive agents — one agent per ad (Phase 3):**

Determine how many ads to deep-dive: default 5 (or fewer if total ads < 5), "top N" if user specified, all if user said "all". Launch one agent per ad. Each agent's prompt must be self-contained:

> Analyze the ad creative at {absolute path to downloaded asset}. Follow the skill instructions in ~/.claude/skills/ad-creative-analysis/SKILL.md exactly: for a video, upload to the Gemini Files API and analyze with gemini-2.5-flash using the video prompt in ~/.claude/skills/ad-creative-analysis/references/prompts.md (GEMINI_API_KEY is in ~/.zshrc — run `source ~/.zshrc` first); for an image, Read the file directly and apply the image prompt. Ad metadata: {CTA URL, dimensions, any known context}. Write the full analysis to {output dir}/deep-dives/ad-{NN}-breakdown.md covering: frame-by-frame/element breakdown, hook effectiveness score, full script/copy transcription, what makes it work, what could be improved. Reply with a 3-line summary.

**One landing-page agent (Phase 4):**

Collect all unique CTA URLs from the extraction metadata, dedupe (many ads → same page), then launch ONE agent that processes them sequentially (they share one Chrome browser — never parallelize browser work across agents):

> For each of these URLs: {list}. Follow ~/.claude/skills/landing-page-analysis/SKILL.md exactly (load Chrome MCP tools via ToolSearch as it describes). Write each analysis to {output dir}/landing-pages/page-{NN}-analysis.md and save screenshots to {output dir}/assets/. Reply with a one-line verdict per page.

**Rules:**
- All agent prompts must use absolute paths — subagents don't inherit your conversation context.
- If more than 8 deep-dive ads, launch them in batches of 8.
- If a subagent fails, retry it once with the error appended to its prompt; if it fails again, note the gap in the master report rather than blocking.

**Checkpoint (after all agents return):** "Completed X deep-dives and Y landing page analyses. Compiling final report..."
